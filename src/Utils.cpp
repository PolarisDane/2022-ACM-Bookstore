...
void BookstoreWork() {
  ...
  while (getline(std::cin, order)) {
    ...
    if (buffer[1] == "su") {
      ...
      for (int i = 2; i <= cnt; i++) {
        ...
        else if (buffer[i][1] == 'n') { BookstoreSys.BookSys.SearchName(buffer[i].substr(6)); }
        else if (buffer[i][1] == 'a') { BookstoreSys.BookSys.SearchAuthor(buffer[i].substr(8)); }
        else if (buffer[i][1] == 'k') { BookstoreSys.BookSys.SearchKeyword(buffer[i].substr(9)); }
      }
    }
      ...
      for (int i = 2; i <= cnt; i++) {
        ...
        else if (buffer[i][1] == 'n') { type = 2; str = buffer[i].substr(6); }
        else if (buffer[i][1] == 'a') { type = 3; str = buffer[i].substr(8); }
        else if (buffer[i][1] == 'k') { type = 4; str = buffer[i].substr(9); }
  ...
  ExitSystem();
}//validating arguments not finished
