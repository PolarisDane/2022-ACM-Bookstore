...
class node {
  ...
  node::node<T>(int siz, int pos, int pre = 0, int nxt = 0)
    :siz(siz), pos(pos), pre(pre), nxt(nxt) {}
};

template<class T>
class LinkList {
  LinkList<T>(const std::string& str) {
  ...
  std::vector<element<T>> find(const std::string& index) {
    std::vector<element<T>> res; res.clear();
    if (!block_cnt) {
      return res;
    }
    node<T> p;
  ...
  void LinkList<BookstoreUser>::modify(const std::string& index, const std::string& curPassword, const std::string& newPassword) {
    ...
    for (int i = 1; i <= block_cnt; i++) {
      ...
      for (int j = 1; j <= p.siz; j++) {
        if (index == p.data[j].index) {
          if (p.data[j].password != curPassword) {
            std::cerr << "wrong password" << std::endl;
            return;
          }
          p.data[j].password = newPassword;
          writeNode(p.pos, p);
          break;
        }
        if (index < p.data[j].index) break;
